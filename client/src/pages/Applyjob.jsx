...
function Applyjob() {
  ...
  const { id } = useParams();
  const { jobs, backend_url, userData, userApplications, fetchUserApplications } = useContext(AppContext)
  // Here we create the state to hold the data 
  const [jobData, setJobData] = useState(null)
  // Here we create the states to check and update the Apply to Applied 
  ...
  const applyHandler = async () => {
    try {
      ...
      if (!userData.resume) {
        ...
        return toast.error("upload resume for apply")
      }
      const token = await getToken()
      const { data } = await axios.post(backend_url + "/api/users/apply",
        { jobId: jobData._id },
